...
from ocr import get_name_fom_image
from text_to_speech import output_audio_from_text
...
def main():
    ...
    parser.add_argument('-image_file_name', type=str, default='data/test.jpg')
    parser.add_argument('-question', type=str, default='What vechile is in the picture?')
    ##
    parser.add_argument('-module', type=str, default='vqa')
    parser.add_argument('-enable_text_to_speech', type=bool, default=False)
    parser.add_argument('-wav_file_path', type=str, default='data/male.wav')
    ##
    args = parser.parse_args()

    ##
    if args.module=='speech_to_text':
        text = get_text_from_speech(args.wav_file_path)
        print(f"text in audio: {text}")
   
    elif args.module=='ocr': 
        text = get_name_fom_image(args.image_file_name)
        print(f"text in image: {text}")
    ##
        
    elif args.module=='vqa':  ##
        ...
        for label in reversed(y_sort_index[0,-5:]):
            ##
            text = str(round(y_output[0,label]*100,2)).zfill(5), "% ", labelencoder.inverse_transform(label)
            print(text)  
            ##
    
    ##
    else:
        text = f"incorrect module: {args.module}"
        print(text)
    
    if args.enable_text_to_speech:
        output_audio_from_text(text)
    ##
